...
const CheckoutScreen = (props: any) => {
  ...
  const onClientSessionActions: OnClientSessionActionsCallback = async (
    ...
    if (currentClientToken) {
      ...
      const clientSession: IClientSession = await setClientSessionActions(
        clientSessionActionsRequestBody
      );
  ...
  const onTokenizeSuccess: OnTokenizeSuccessCallback = async (
    ...
    try {
      ...
      console.log(payment.requiredAction?.name);
  ...
  const onUniversalCheckoutButtonTapped = async () => {
    try {
      ...
      console.log("client token", currentClientToken);

      //@ts-ignore
      Primer.showUniversalCheckout(currentClientToken, {
        onTokenizeSuccess,
      });

      // show Universal Checkout with above client token and config.
      // Primer.showUniversalCheckout(currentClientToken, {
      // onTokenizeSuccess,
      // onResumeSuccess,
      // onError: onError,
  ...
  const onTokenizeSuccess2 = async (t, resumeHandler) => {
    const payment = await createPayment(t);
    resumeHandler.handleSuccess(); // resume the checkout flow with success message.
  };
  const onApplePayButtonTapped = async () => {
    try {
      ...
      //@ts-ignore
      Primer.showPaymentMethod(currentClientToken, intent, {
        onTokenizeSuccess,
        onResumeSuccess,
      });
    } catch (err) {
